...
Read these files **only when your task requires them**. They are in the `references/` directory next to this file.

| File | When to read |
|------|-------------|
| `references/components.md` | Adding or modifying components (callouts, cards, steps, tabs, accordions, code groups, fields, frames, icons, tooltips, badges, trees, mermaid, MDX, panels, prompts, colors, tiles, updates, views). Also covers table column widths. |
...
Before substantial content work, read `references/product-context.md` and check for `.mintlify/product-brief.md`.
...
For the complete list including `searchable`, `boost`, `deprecated`, `related`, `groups`, and more, read `references/configuration.md`.

## Quick component reference

Below are the most commonly used components. For full props and all 26 components, read `references/components.md`.
...
Install with `npm i -g mint`. Key commands: `mint dev` (local preview), `mint validate`, `mint broken-links`, `mint a11y`, `mint score`, `mint automations`, `mint new`, `mint signup`, `mint index` (install the Mintlify Index MCP server in supported coding agents). Read `references/cli.md` for full flags and subcommands.
